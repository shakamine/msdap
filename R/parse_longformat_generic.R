...
#'
#' You can also import the report.tsv file generated by Proteoscape using the `import_dataset_diann`
#' function, which is a lot faster (because we don't have to post-process the Proteoscape-specific output files).
#'
#' @param filename_peptides the full file path of the peptides parquet file
#' @param filename_proteins the full file path of the proteins parquet file
...
#' @export
import_dataset_proteoscape = function(filename_peptides, filename_proteins, use_normalized_intensities = FALSE, confidence_threshold = 0.01, use_irt = TRUE, select_unique_precursor_per_modseq = TRUE, do_plot = TRUE) {
  # first, check if input files exist
  if(!file.exists(filename_peptides)) {
    append_log(sprintf('Cannot find file "%s"', filename_peptides), type = "error")
  }
  if(!file.exists(filename_proteins)) {
    append_log(sprintf('Cannot find file "%s"', filename_proteins), type = "error")
  }

  reset_log()
  append_log("reading Proteoscape data...", type = "info")

  pep = arrow::read_parquet(
    filename_peptides,
    col_select = NULL,
    as_data_frame = TRUE,
  ...
  if(!"protein_group_parent_id" %in% colnames(pep)) {
    append_log("expected the column 'protein_group_parent_id' in the peptide.parquet file", type = "error")
  }
  if(!"protein_group_id" %in% colnames(prot)) {
    append_log("expected the column 'protein_group_id' in the protein.parquet file", type = "error")
  }

  i = match(pep$protein_group_parent_id, prot$protein_group_id)
  if(anyNA(i)) {
    append_log("some peptide protein_group_parent_id are missing the in protein data table", type = "error")
  }
  pep$protein_id = prot$protein_group_name[i] # this column/parameter matches our DIA-NN parser
  # pep$protein_id = prot$protein_accession[i] # contains more data than DIA-NN protein group identifiers, i.e. not the same result as data we by default extract from DIA-NN's report.tsv
  rm(prot, i)

  tmp = setdiff(c("stripped_peptide", "ptm_locations", "ptms"), colnames(pep))
  if(length(tmp) > 0) {
    append_log(paste("expected the column(s) in the peptide.parquet file are missing;", paste(tmp, collapse = ", ")), type = "error")
  }


  # compose sequence_modified from columns stripped_peptide, ptms, ptm_locations
  pep$sequence_modified = pep$stripped_peptide
  for(row in which(lengths(pep$ptms) > 0)) {
    s = pep$stripped_peptide[row]
    su = unlist(strsplit(s, "", fixed = TRUE), FALSE, FALSE)
    su_modid = pep$ptms[[row]]
    su_modindex = pep$ptm_locations[[row]] + 1 # input data is zero indexed, R is 1 indexed
    for(i in seq_along(su_modid)) {
      if(su_modindex[i] < 1) {
        su[1] = paste0("(", su_modid[i], ")", su[1])
      } else {
        su[su_modindex[i]] = paste0(su[su_modindex[i]], "(", su_modid[i], ")")
      }
    }
    pep$sequence_modified[row] = paste(su, collapse = "")
  }

  # tibble(seq=pep$stripped_peptide, seqmod=pep$sequence_modified) |> filter(seq != seqmod) |> distinct_all()

  ds = import_dataset_in_long_format(filename = NULL,
                                     x = pep,
                                     attributes_required = list(sample_id = "sample_name",
                                                                protein_id = "protein_id",
                                                                sequence_modified = "sequence_modified",
                                                                sequence_plain = "stripped_peptide",
                                                                charge = "precursor_charge",
                                                                rt = ifelse(use_irt, "irt", "rt"),
                                                                intensity = ifelse(use_normalized_intensities, "precursor_quantity_normalised", "precursor_quantity"),
                                                                confidence = "precursor_q_value",
                                                                confidence_score = "evidence"),
                                     select_unique_precursor_per_modseq = select_unique_precursor_per_modseq,
...
#' @param do_plot logical indicating whether to create QC plots that are shown in the downstream PDF report (if enabled)
#' @export
import_dataset_openswath = function(filename, confidence_threshold = 0.01, select_unique_precursor_per_modseq = TRUE, return_decoys = FALSE, do_plot = TRUE) {
  reset_log()
...
#' @param do_plot logical indicating whether to create QC plots that are shown in the downstream PDF report (if enabled)
#' @export
import_dataset_skyline = function(filename, acquisition_mode, confidence_threshold = 0.01, collapse_peptide_by = "sequence_modified", select_unique_precursor_per_modseq = (acquisition_mode=="dia"), return_decoys = FALSE, do_plot = TRUE) {
  reset_log()
...
#' @param do_plot logical indicating whether to create QC plots that are shown in the downstream PDF report (if enabled)
#' @export
import_dataset_diann = function(filename, confidence_threshold = 0.01, use_normalized_intensities = FALSE, use_irt = TRUE, select_unique_precursor_per_modseq = TRUE, do_plot = TRUE) {
  reset_log()
  append_log("reading DIA-NN report...", type = "info")

  rt_col = ifelse(use_irt, "iRT", "RT")

  # if the filename ends with ".parquet", use the Apache arrow library to read the report in parquet format
  f = df = NULL
  ...
  ds = import_dataset_in_long_format(
    filename = f,
    x = df,
...
import_dataset_spectronaut = function(filename, confidence_threshold = 0.01, use_normalized_intensities = FALSE, use_irt = TRUE,
                                      remove_shared_spectronaut_proteingroups = FALSE, select_unique_precursor_per_modseq = TRUE, return_decoys = FALSE, do_plot = TRUE) {
  reset_log()
  ...
  ds = import_dataset_in_long_format(
    ...
    attributes_required = list(sample_id = "R.FileName",
                               ...
                               rt = rt_col,
                               isdecoy = "EG.IsDecoy",
                               intensity = c("FG.MS2PeakArea", "FG.TotalPeakArea", "FG.MS2RawQuantity", "FG.Quantity", "FG.MS2Quantity"),
                               cscore = "EG.Cscore",
                               confidence = "EG.Qvalue"),
    attributes_optional = list(spectral_library = "EG.Library",
                               mz = "FG.PrecMzCalibrated",
                               ptm_localization = "EG_ProteinPTMLocations",
                               ptm_probability = "EG_PTMLocalizationProbabilities",
...
import_dataset_in_long_format = function(filename = NULL, x = NULL, attributes_required=list(), attributes_optional = list(), confidence_threshold = 0.01,
  ...
  if(!"confidence" %in% colnames(DT)) {
    ...
    if(!is.numeric(DT$confidence)) {
      DT$confidence = suppressWarnings(as.numeric(DT$confidence))
    }
  ...
  if("intensity_norm" %in% colnames(DT)) {
    if(!is.numeric(DT$intensity_norm)) {
      DT$intensity_norm = suppressWarnings(as.numeric(DT$intensity_norm))
    }
