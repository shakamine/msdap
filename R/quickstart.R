...
#' @param groups_min_pass minimum number of groups that must pass peptide filtering in order to retain data in the `all_group` and `by_contrast` results created by `filter_dataset()`. Set to `NA` (default) to preserve current behavior; numeric values `>= 1` relax only `all_group` and `by_contrast`, not `by_group`.
...
analysis_quickstart = function(
  ...
  dataset = filter_dataset(dataset,
                           ...
                           groups_min_pass = groups_min_pass)
